...
#           training_params['step'])
    print("[epoch {}][{}/{}] loss: {:1.4f} PSNR_train: {:1.4f}".format(
        epoch + 1, idx + 1, num_minibatches, loss.item(), 0.0))
...
def save_model_checkpoint(model, argdict, optimizer, train_pars, epoch):
    ...
    if epoch % argdict['save_every_epochs'] == 0:
        torch.save(save_dict,
                   os.path.join(argdict['log_dir'], 'ckpt_e{}.pth'.format(epoch + 1)))
    del save_dict
...
def validate_and_log(model_temp, dataset_val, valnoisestd, temp_psz, writer,
    ...
    with torch.no_grad():
        ...
        t2 = time.time()
        print("[epoch %d] PSNR_val: %.4f, on %.2f sec" % (epoch + 1, psnr_val,
                                                          (t2 - t1)))
    ...
    try:
        ...
        irecon = tutils.make_grid(out_val.data[idx].clamp(0., 1.),
                                  ...
                                  normalize=False,
                                  scale_each=False)
        writer.add_image('Reconstructed validation image {}'.format(idx), irecon,
                         epoch)
