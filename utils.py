...
def normalize_augment(datain, ctrl_fr_idx):
    ...
    def transform(sample):
        ...
        rot180_flipud = lambda x: torch.flip(torch.rot90(x, k=2, dims=[2, 3]),
                                             dims=[2])
        rot180_flipud.__name__ = 'rot180_flipud'
        rot270 = lambda x: torch.rot90(x, k=3, dims=[2, 3])
        rot270.__name__ = 'rot270'
        rot270_flipud = lambda x: torch.flip(torch.rot90(x, k=3, dims=[2, 3]),
                                             dims=[2])
        ...
        aug_list = [
            do_nothing, flipud, rot90, rot90_flipud, rot180, rot180_flipud, rot270,
            rot270_flipud, add_csnt
        ]
        w_aug = [32, 12, 12, 12, 12, 12, 12, 12,
                 12]  # one fourth chances to do_nothing
...
def svd_orthogonalization(lyr):
    ...
    if classname.find('Conv') != -1:
        ...
        weights = weights.permute(2, 3, 1,
                                  0).contiguous().view(f1 * f2 * c_in, c_out)
        ...
        try:
            ...
            lyr.weight.data = weights.view(f1, f2, c_in,
                                           c_out).permute(3, 2, 0,
                                                          1).type(dtype)
        except:
            pass
